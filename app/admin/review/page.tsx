...
export default function ReviewPage() {
      ...
      try {
        ...
        if (savedRows) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            // Check if savedRows contains stale mock items
            const isMockData = parsed.some(
              (item: any) =>
                item.name === "Plain Baseball Cap" ||
                item.name === "Vintage Bucket Hat" ||
                item.name === "Premium Snapback" ||
                item.name === "Winter Knit Beanie" ||
                item.name === "Baseball Cap Classic"
            );
            if (!isMockData) {
              setRows(parsed);
              setIsLoading(false);
        ...
        if (raw) {
          ...
          // No drafts in session - start empty with no mock data
          setRows([]);
        }
      } catch (err: any) {
  ...
  // Batch Action: Delete Selected Items from Review
  const handleDeleteSelected = () => {
    if (selectedIndices.length === 0) return;
    const count = selectedIndices.length;
    updateRowsAndSync((prev) => {
      const remaining = prev.filter((_, idx) => !selectedIndices.includes(idx));
      if (remaining.length === 0) {
        sessionStorage.removeItem("drafts");
        sessionStorage.removeItem("smartcap_review_rows");
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden md:flex justify-end py-2">
        <div className="inline-flex items-center rounded-none border border-[#DED9CF] bg-white p-1 shadow-2xs">
          <button
            ...
            className={`rounded-none px-5 py-1.5 text-xs font-bold transition-all cursor-pointer ${viewMode === "grid"
          ...
          <button
            ...
            className={`rounded-none px-5 py-1.5 text-xs font-bold transition-all cursor-pointer ${viewMode === "table"
                ? "bg-[#D8D4CD] text-[#1F2022] font-black"
                : "text-[#1F2022]/70 hover:text-[#1F2022]"
              }`}
          >
      ...
      {message && (
        <div
          className={`rounded-none border p-4 text-xs font-bold shadow-xs ${message.type === "success"
        ...
        <div className="space-y-4">
          <div className={`flex gap-3 min-h-[48px] ${selectedIndices.length === 0 ? "items-center flex-row" : "flex-col"}`}>
            ...
            <button
              ...
              className={`flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-none border border-[#DED9CF] transition-all cursor-pointer shadow-2xs ${isAllSelected
            ...
            {selectedIndices.length > 0 ? (
              ...
              <div className="flex flex-wrap items-center gap-3 animate-in fade-in slide-in-from-left-2 duration-200">
                ...
                <button
                  ...
                  className="flex h-10 w-28 items-center justify-center gap-1.5 rounded-none bg-stone-100 text-stone-700 border border-stone-300 hover:bg-red-50 hover:text-red-700 hover:border-red-300 text-xs font-extrabold shadow-xs cursor-pointer transition-colors"
                  title="Remove selected items from review"
                >
                  ...
                  {isDateDropdownOpen && (
                    <div className="absolute right-0 top-10 z-50 min-w-[180px] rounded-none border border-[#DED9CF] bg-white shadow-xl py-1 animate-in fade-in zoom-in-95 duration-150">
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-xs font-bold transition-colors hover:bg-[#FCFAF7] cursor-pointer flex items-center justify-between ${selectedDate === "All" ? "bg-[#D8D4CD] text-[#1F2022] font-black" : "text-[#1F2022]"
                          }`}
                      >
                        <span>All Dates</span>
                      ...
                      {availableDates.map((dateStr) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 text-xs font-bold transition-colors hover:bg-[#FCFAF7] cursor-pointer flex items-center justify-between ${selectedDate === dateStr ? "bg-[#D8D4CD] text-[#1F2022] font-black" : "text-[#1F2022]"
                              }`}
                          >
                            <span>{dateStr}</span>
                    ...
                    <div className="absolute right-0 top-10 z-50 min-w-[130px] rounded-none border border-[#DED9CF] bg-white shadow-xl animate-in fade-in zoom-in-95 duration-150">
                      {(["all", "Pending", "Approved", "Rejected"] as const).map((st) => (
                        <button
                          ...
                          className={`w-full text-center px-4 py-2 text-[10px] sm:text-xs font-bold border-b border-[#E5E2DC] last:border-0 transition-colors cursor-pointer capitalize ${statusFilter === st ? "bg-[#E5E2DC] text-[#1F2022]" : "text-[#1F2022] hover:bg-[#FCFAF7]"
                            }`}
                        >
            ...
            {(["all", "Pending", "Approved", "Rejected"] as const).map((st) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-none px-3.5 py-1 text-xs font-bold transition-all cursor-pointer capitalize ${active
                  ...
                  <div key={realIndex} className="relative mt-2 bg-transparent">
                    ...
                    <div className="absolute -left-2 -top-2 z-10 bg-white">
                      <button
                        ...
                        className={`flex h-5 w-5 items-center justify-center rounded-none border transition-all cursor-pointer ${isChecked
                            ? "border-[#353B2D] bg-[#353B2D] text-white"
                    ...
                    <div className={`flex flex-col rounded-none bg-white p-3 pt-5 border transition-all ${isChecked ? "border-[#353B2D] shadow-xs" : "border-[#DED9CF]"
                      }`}>
                          ...
                          <div className="flex justify-between items-end mt-1">
                            ...
                            <div className="relative status-dropdown-container">
                              <button
                                ...
                                className={`flex items-center justify-center rounded-none px-2 py-0.5 text-[9px] font-bold transition-transform cursor-pointer ${row.status === "Approved"
                                ...
                                <div className="absolute right-0 top-full mt-1 z-50 min-w-[100px] rounded-none border border-[#DED9CF] bg-white shadow-xl py-1 animate-in fade-in zoom-in-95 duration-150">
                                  {(["Pending", "Approved", "Rejected"] as ReviewStatus[]).map((statusOption) => (
                                    <button
                                      ...
                                      className={`w-full text-left px-2 py-1 text-[9px] font-extrabold transition-colors cursor-pointer flex items-center justify-between ${row.status === statusOption
                                          ? "bg-[#D8D4CD] text-[#1F2022] font-black"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <div
                        key={realIndex}
                        className={`relative flex flex-col overflow-hidden rounded-none border bg-white p-5 transition-all space-y-4 shadow-xs hover:shadow-md ${isChecked ? "border-[#353B2D] ring-1 ring-[#353B2D]" : "border-[#E5E2DC]"
                          }`}
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <button
                            type="button"
                            onClick={() => toggleSelect(realIndex)}
                            className={`flex h-4 w-4 items-center justify-center rounded-none border transition-all cursor-pointer ${isChecked
                                ? "border-[#353B2D] bg-[#353B2D] text-white"
                                : "border-[#DED9CF] bg-white text-transparent hover:border-[#353B2D]"
                              }`}
                            title="Select item"
                          ...
                          <div className="relative status-dropdown-container">
                            <button
                              type="button"
                              onClick={() => setOpenStatusIndex(openStatusIndex === realIndex ? null : realIndex)}
                              className={`flex items-center gap-1.5 rounded-none px-3 py-1 text-[11px] font-extrabold transition-all cursor-pointer ${row.status === "Approved"
                              ...
                              <div className="absolute right-0 top-full mt-1 z-50 min-w-[130px] rounded-none border border-[#DED9CF] bg-white shadow-xl py-1 animate-in fade-in zoom-in-95 duration-150">
                                {(["Pending", "Approved", "Rejected"] as ReviewStatus[]).map((statusOption) => (
                                  <button
                                    ...
                                    className={`w-full text-left px-3 py-1.5 text-[11px] font-extrabold transition-colors cursor-pointer flex items-center justify-between ${row.status === statusOption
                    ...
                    return (
                      <div key={realIndex} className="flex items-center gap-4">
                        ...
                        <button
                          type="button"
                          onClick={() => toggleSelect(realIndex)}
                          className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-none border transition-all cursor-pointer ${isChecked
                              ? "border-[#353B2D] bg-[#353B2D] text-white"
                              : "border-[#DED9CF] bg-white text-transparent hover:border-[#353B2D]"
                            }`}
                          title="Select item"
                        ...
                        <div
                          className={`flex flex-col md:flex-row items-stretch gap-6 flex-1 rounded-none border bg-white p-6 transition-all ${isChecked ? "border-[#353B2D] shadow-xs" : "border-[#DED9CF]"
                            }`}
                        >
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="relative status-dropdown-container">
                                <button
                                  type="button"
                                  onClick={() => setOpenStatusIndex(openStatusIndex === realIndex ? null : realIndex)}
                                  className={`flex items-center gap-1.5 rounded-none px-3 py-1 text-[11px] font-extrabold transition-transform active:scale-95 cursor-pointer shadow-2xs ${row.status === "Approved"
