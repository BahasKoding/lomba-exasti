...
Locator: getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-plus') })
Expected pattern: /.+/
Received string:  ""
Timeout: 5000ms

Call log:
  - Expect "soft toHaveAccessibleName" getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-plus') }) with timeout 5000ms
  - waiting for getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-plus') })
...
Locator: getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-minus') })
Expected pattern: /.+/
Received string:  ""
Timeout: 5000ms

Call log:
  - Expect "soft toHaveAccessibleName" getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-minus') }) with timeout 5000ms
  - waiting for getByRole('main').getByRole('button', { name: '', exact: true }).filter({ has: locator('svg.lucide-minus') })
    14 × locator resolved to <button type="button" class="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-none border border-[#353B2D] text-[#353B2D] transition-all duration-300 hover:bg-[#353B2D] hover:text-white cursor-pointer active:scale-95">…</button>
       - unexpected value ""
